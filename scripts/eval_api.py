#!/usr/bin/env python3
"""
Batch evaluation script for YiCeNet RL training.
Reads samples from file, sends to any OpenAI-compatible API for evaluation,
saves results as JSONL.

Config via environment variables:
  EVAL_API_URL   — API endpoint (default: https://api.deepseek.com/v1/chat/completions)
  EVAL_MODEL     — Model name       (default: deepseek-chat)
  EVAL_API_KEY   — API key (fallback: DEEPSEEK_API_KEY for backward compat)
"""
import json
import os
import time
...
# Load API key: EVAL_API_KEY env var → DEEPSEEK_API_KEY env var → .env files
    ...
    for env_candidate in [
        ...
        if env_candidate.exists():
            with open(env_candidate) as f:
                for line in f:
                    line = line.strip()
                    if line.startswith("EVAL_API_KEY="):
...
if not api_key:
    sys.exit("ERROR: EVAL_API_KEY (or DEEPSEEK_API_KEY) not found. "
             "Set via env var or project .env file.")
...
def evaluate_batch(samples, batch_id):
    """Evaluate a batch of samples via the configured API."""
    sample_lines = []
    for i, s in enumerate(samples):
        text = s["user_text"][:300]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Batch evaluation via OpenAI-compatible API"
    )
    ...
    print(f"API: {API_URL}  Model: {MODEL}", file=sys.stderr)
    ...
    if all_results:
        mean_sat = sum(r['satisfaction'] for r in all_results) / len(all_results)
        print(f"\nSaved {len(all_results)} evaluations to {args.output}", file=sys.stderr)
        print(f"Stats: mean_satisfaction={mean_sat:+.3f}")
    else:
        print(f"\nNo evaluations saved to {args.output}", file=sys.stderr)
