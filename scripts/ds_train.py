#!/usr/bin/env python3
"""
DS-supervised YiCeNet training pipeline.
Reads buffer samples + DS evaluations → trains World Model → RL fine-tune → v14-r{N}
"""
...
YICENET_ROOT = Path.home() / "YiCeNet"
...
from src.world_model import WorldModelV2, power_law_weight
from src.rl_train import project_to_hexagram_space, compute_hexagram_reward
...
def load_ds_results(path: Path) -> dict[int, dict]:
    """Load DS evaluation results keyed by msg_id."""
    results = {}
    with open(path) as f:
        for line in f:
            r = json.loads(line)
...
def ds_supervised_wm_training(
    wm: WorldModelV2,
    model: YiCeNet,
    samples: list[dict],
    ds_results: dict[int, dict],
    config: YiCeNetConfig,
    device: str,
    endogenous: bool = False,
) -> WorldModelV2:
    """Train World Model with DS-supervised targets."""
    wm.train()
    ...
    print(f"  Training WM on {len(samples)} DS-supervised samples...")
    ...
    for idx, s in enumerate(samples):
        ...
        if msg_id not in ds_results:
            continue
        
        ds = ds_results[msg_id]
        ...
        # Target hexagram distribution from DS signals
        reward_sig = {
        ...
        # Target external vector with DS satisfaction score
    ...
    print(f"  DS-supervised WM training done: {total_count} samples, "
...
def rl_fine_tune_v14(
    ...
    """RL fine-tuning with 64-dim projection reward using DS-supervised WM."""
    wm.eval()
    for p in wm.parameters():
        p.requires_grad = False
    
    # Build training pairs (sample + DS result)
    train_pairs = []
    for s in samples:
        msg_id = s.get("msg_id", 0)
        if msg_id in ds_results:
            train_pairs.append((s, ds_results[msg_id]))
    ...
    for ep in range(episodes):
        ...
        # Reward = similarity between WM prediction and DS target
    ...
    # Save
    out_path = CHECKPOINT_DIR / f"yicenet_v14_{version}.pt"
    torch.save({
        "model_state_dict": model.state_dict(),
        "version": f"v14_{version}",
    ...
    # Also save as latest
    latest_path = CHECKPOINT_DIR / "yicenet_v14_latest.pt"
    torch.save({
        "model_state_dict": model.state_dict(),
        "version": f"v14_{version}",
        "avg_reward": avg_reward,
...
def main():
    ...
    parser.add_argument("--ds-results", required=True, help="DS evaluation .jsonl path (relative to YICENET_ROOT)")
    ...
    ds_path = YICENET_ROOT / args.ds_results
    
    print(f"\n{'='*60}")
    print(f"DS-Supervised Training: v14_{args.version}")
    print(f"{'='*60}")
    ...
    ds_results = load_ds_results(ds_path)
    print(f"DS results: {len(ds_results)}")
    
    # Filter samples with DS results
    valid = [(i, s) for i, s in enumerate(samples) if s.get("msg_id", 0) in ds_results]
    ...
    # Step 1: DS-supervised WM training
    print(f"\n--- Step 1: DS-supervised World Model training ---")
    wm = ds_supervised_wm_training(wm, model, samples, ds_results, config, DEVICE, endogenous=args.endogenous)
    ...
    print(f"\n--- Step 2: RL fine-tuning ---")
    model = rl_fine_tune_v14(model, wm, samples, ds_results, config, DEVICE, args.version)
