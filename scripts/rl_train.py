...
from yicenet.world_model import WorldModelV2, WorldModelV3, power_law_weight
...
def flywheel_entry_to_context_vector(entry: dict) -> torch.Tensor:
    """Map a flywheel buffer entry to a 27-dim context vector for WMv3 training."""
    user_text = entry.get("user_text", "")
    token_cost = entry.get("token_cost", 0)
    satisfaction = entry.get("satisfaction", 0.0)

    vec = [
        min(len(user_text) / 512.0, 1.0),                    # tok_user_input_len
        0.0,                                                   # tok_is_first_turn
        token_cost / 4096.0,                                   # tok_prompt_tokens
        token_cost * 0.3 / 4096.0,                             # tok_completion_tokens
        0.5,                                                   # tok_api_duration
        0.0,                                                   # tok_tool_count
        1.0,                                                   # tok_tool_success_rate
        0.0,                                                   # tok_tool_retry_count
        0.0,                                                   # tok_tool_duration
        0.0,                                                   # tok_tool_output_size
        0.0,                                                   # tok_tool_diversity
        token_cost * 0.01 / 4000.0,                            # tok_response_len
        float("```" in user_text),                             # tok_has_code
        0.0,                                                   # tok_code_block_count
        0.5,                                                   # tok_hex_conf
        0.0,                                                   # tok_hex_q_gap
        0.5,                                                   # tok_hex_entropy
        0.3,                                                   # tok_user_speed
        1.0,                                                   # tok_user_speed_ratio
        satisfaction - 0.5,                                    # tok_mood_trend
        0.0,                                                   # tok_drift_trend
        0.0,                                                   # tok_is_prev_correction
        0.0,                                                   # tok_is_prev_praise
        satisfaction,                                          # tok_user_satisfaction
        float(entry.get("corrected", False)),                  # tok_is_correction
        float(entry.get("praised", False)),                    # tok_is_praise
        float(entry.get("abandoned", False)),                  # tok_is_abandon
    ]
...
def supervised_wm_training(
    ...
    """Train World Model V3 with Supervised targets."""
    wm.train()
    ...
    print(f"  Training WMv3 on {len(samples)} Supervised samples...")
    ...
    for idx, s in enumerate(samples):
        ...
        # Build context vector from flywheel entry + eval satisfaction
        entry_for_ctx = dict(s)
        entry_for_ctx["satisfaction"] = satisfaction
        ctx_vec = flywheel_entry_to_context_vector(entry_for_ctx).to(device)
        ...
        # HeadA mask: only contribute when hexagram_evolution present
        ...
        # Forward through WMv3
        pred_dist, pred_ext = wm(probes.unsqueeze(0), ctx_vec.unsqueeze(0), hex_id.unsqueeze(0))

        # Weighted loss with HeadA mask
        kl = F.kl_div(
    ...
    print(f"  Supervised WMv3 training done: {total_count} samples, "
...
def rl_fine_tune_v14(
    ...
    """RL fine-tuning with 64-dim projection reward using Supervised WMv3."""
    wm.eval()
    ...
    for ep in range(episodes):
        ...
        # WM prediction via V3
        with torch.no_grad():
            wm_dist, _ = wm(probes.to(device).unsqueeze(0), ctx_vec.unsqueeze(0), hex_id.to(device))
...
def main():
    ...
    if wm_path.exists():
        try:
            ...
            print(f"Loaded WMv3: {wm_path}")
        except (KeyError, RuntimeError) as e:
            print(f"  Old WM format ({e}), starting fresh V3")
            wm = WorldModelV3().to(DEVICE)
    else:
        wm = WorldModelV3().to(DEVICE)
        print("Fresh WMv3 (no checkpoint)")

    # Step 1: Supervised WM training
    print(f"\n--- Step 1: Supervised World Model V3 training ---")
    ...
    wm.save(str(wm_path))
    print(f"  WMv3 saved: world_model_v14_{args.version}.pt")
